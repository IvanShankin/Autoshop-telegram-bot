...
from src.application.products.accounts.tg.dto.schemas import BaseAccountProcessingResult, ArchiveProcessingResult
...
class TestImportAccount:
    ...
    async def test_import_creates_product_account(self, tmp_path, create_category):
        ...
        """
        from src.application.products.accounts.tg import _input_account
    ...
    async def test_import_with_duplicate_accounts_archived(self, tmp_path, create_category):
        ...
        with patch.object(
            input_account,
            "check_valid_accounts_telethon",
            new=AsyncMock(side_effect=fake_check_dup)
        ):
            gen = import_telegram_accounts_from_archive(
                str(input_zip),
                category_id=category.category_id,
...
async def test_import_in_db_valid_and_invalid(tmp_path, create_category):
    from src.application.products.accounts.tg import _input_account
    from src.application.products.accounts.tg._input_account import import_in_db
...
async def test_split_unique_and_duplicates_basic():
    from src.application.products.accounts.tg._input_account import split_unique_and_duplicates
...
async def test_process_duplicates_calls_make_archive(tmp_path):
    from src.application.products.accounts.tg import _input_account
    from src.application.products.accounts.tg._input_account import process_inappropriate_acc
...
async def test_process_archives_batch_no_archives(tmp_path):
    from src.application.products.accounts.tg._input_account import process_archives_batch
...
async def test_process_archives_batch_calls_process_single_archive(tmp_path):
    ...
    with patch.object(
        ...
        mock_proc.return_value = ArchiveProcessingResult(valid=True, archive_path=str(archive), dir_path=str(tmp_path),
                                                         user=None)
...
async def test_process_dirs_batch_no_dirs(tmp_path):
    from src.application.products.accounts.tg._input_account import process_dirs_batch
...
async def test_process_single_dir_valid(tmp_path):
    from src.application.products.accounts.tg import _input_account
    from src.application.products.accounts.tg._input_account import process_single_dir
...
async def test_process_single_archive_valid(tmp_path):
    from src.application.products.accounts.tg import _input_account
    from src.application.products.accounts.tg._input_account import process_single_archive
