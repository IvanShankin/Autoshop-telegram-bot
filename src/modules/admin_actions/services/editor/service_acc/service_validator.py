    ...
    if not service:
        if callback:
            try:
                ...
                pass
            await callback.answer(get_text(user.language, "admins_editor","The service_acc is no longer available"), show_alert=True)
            return

        await send_message(chat_id=user.user_id, message=get_text(user.language, "admins_editor","The service_acc is no longer available"))
        return
