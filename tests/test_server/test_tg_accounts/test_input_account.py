...
from src.services.tg_accounts.shemas import BaseAccountProcessingResult, ArchiveProcessingResult
...
class TestImportAccount:
    ...
    async def test_import_creates_product_account(self, tmp_path, create_account_category):
        ...
        with patch("src.services.tg_accounts.input_account.check_valid_accounts_telethon", new=AsyncMock(side_effect=fake_check)):
    ...
    async def test_import_with_duplicate_accounts_archived(self, tmp_path, create_account_category):
        ...
        with patch("src.services.tg_accounts.input_account.check_valid_accounts_telethon", new=AsyncMock(side_effect=fake_check_dup)):
            gen = import_telegram_accounts_from_archive(str(input_zip), account_category_id=category.account_category_id, type_account_service="telegram")
            result = await gen.__anext__()
    ...
    async def test_import_with_invalid_accounts_archived(self, tmp_path, create_account_category):
        ...
        with patch(
                "src.services.tg_accounts.input_account.check_valid_accounts_telethon",
...
async def test_import_in_db_valid_and_invalid(tmp_path, create_account_category):
    from src.services.tg_accounts.input_account import import_in_db
    ...
    # Моки DB и файлов
    with patch("src.services.tg_accounts.input_account.encrypted_tg_account", new_callable=AsyncMock) as mock_enc, \
            patch("src.services.tg_accounts.input_account.make_archive", new_callable=AsyncMock) as mock_make_archive:
        # Настроим шифрование: первый успех, второй провал
        mock_enc.side_effect = [
            SimpleNamespace(result=True, encrypted_key_b64="k", encrypted_key_nonce="n", checksum="c"),
...
async def test_split_unique_and_duplicates_basic(create_type_account_service):
    from src.services.tg_accounts.input_account import split_unique_and_duplicates
    ...
    items = [
        SimpleNamespace(valid=True, user=user_1, dir_path="a"),
        SimpleNamespace(valid=True, user=user_2, dir_path="b"),
        SimpleNamespace(valid=True, user=user_3, dir_path="c"),  # дубликат
    ]
...
async def test_process_duplicates_calls_make_archive(tmp_path):
    from src.services.tg_accounts.input_account import process_inappropriate_acc
    items = [SimpleNamespace(dir_path=str(tmp_path / f"dup{i}")) for i in range(2)]
    for item in items:
        Path(item.dir_path).mkdir()

    with patch("src.services.tg_accounts.input_account.make_archive", new_callable=AsyncMock) as mock_make:
...
async def test_process_archives_batch_no_archives(tmp_path):
    from src.services.tg_accounts.input_account import process_archives_batch
...
async def test_process_archives_batch_calls_process_single_archive(tmp_path):
    ...
    with patch("src.services.tg_accounts.input_account.process_single_archive", new_callable=AsyncMock) as mock_proc:
...
async def test_process_dirs_batch_no_dirs(tmp_path):
    from src.services.tg_accounts.input_account import process_dirs_batch
...
async def test_process_dirs_batch_calls_process_single_dir(tmp_path):
    from src.services.tg_accounts.input_account import process_dirs_batch
    (tmp_path / "dir1").mkdir()
    with patch("src.services.tg_accounts.input_account.process_single_dir", new_callable=AsyncMock) as mock_proc:
...
async def test_process_single_dir_valid(tmp_path):
    from src.services.tg_accounts.input_account import process_single_dir
    (tmp_path / "acc").mkdir()
    user = SimpleNamespace(id=1, phone="123")
    with patch("src.services.tg_accounts.input_account.check_valid_accounts_telethon", return_value=user):
...
async def test_process_single_dir_invalid(tmp_path):
    from src.services.tg_accounts.input_account import process_single_dir
    (tmp_path / "acc").mkdir()
    with patch("src.services.tg_accounts.input_account.check_valid_accounts_telethon", return_value=None):
        result = await process_single_dir(str(tmp_path / "acc"))
...
async def test_process_single_archive_valid(tmp_path):
    from src.services.tg_accounts.input_account import process_single_archive
    archive = tmp_path / "a.zip"
    archive.write_text("")
    user = SimpleNamespace(id=1, phone="123")

    with patch("src.services.tg_accounts.input_account.extract_archive_to_temp", return_value=str(tmp_path)), \
         patch("src.services.tg_accounts.input_account.check_valid_accounts_telethon", return_value=user):
...
async def test_process_single_archive_invalid(tmp_path):
    ...
    with patch("src.services.tg_accounts.input_account.extract_archive_to_temp", return_value=str(tmp_path)), \
         patch("src.services.tg_accounts.input_account.check_valid_accounts_telethon", return_value=None):
        result = await process_single_archive(str(archive))
        assert not result.valid