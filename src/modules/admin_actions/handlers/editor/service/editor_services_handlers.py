...
async def add_account_service(callback: CallbackQuery, user: Users):
    await edit_message(
        chat_id=callback.from_user.id,
        message_id=callback.message.message_id,
        message=get_text(user.language, "admins_editor","Select services type"),
...
async def select_type_service(callback: CallbackQuery, state: FSMContext, user: Users):
    ...
    await edit_message(
        chat_id=callback.from_user.id,
        message_id=callback.message.message_id,
        message=get_text(user.language, "admins_editor",'Enter a name for the new services')
    )
...
async def get_service_name(message: Message, state: FSMContext, user: Users):
    ...
    try:
        ...
        await send_message(user.user_id, get_text(user.language, "admins_editor","Unable to create. This services type is in use"))
        return
...
async def service_confirm_delete(callback: CallbackQuery, user: Users):
    ...
    await edit_message(
        ...
        message=get_text(
            ...
            "Before deleting, make sure that this services does not contain categories!"
        ),
...
async def delete_acc_service(callback: CallbackQuery, user: Users):
    ...
    try:
        ...
        message = get_text(user.language, "admins_editor","The services has categories, delete them first")
        reply_markup = back_in_service_kb(user.language, service_id)
