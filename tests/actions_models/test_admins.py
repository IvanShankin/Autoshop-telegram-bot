...
from tests.fixtures.helper_fixture import create_admin_fix

@pytest.mark.asyncio
async def test_check_admin(create_admin_fix):
    assert await check_admin(create_admin_fix.user_id)


@pytest.mark.asyncio
async def test_create_admin_creates_new_admin_and_message(create_new_user):
    # вызываем тестируемую функцию
    admin = await create_admin_fun(create_new_user.user_id)

    # проверяем что admin создан
    assert isinstance(admin, Admins)
    assert admin.user_id == create_new_user.user_id

    # проверяем что MessageForSending тоже создалась
    async with get_db() as session_db:
        result = await session_db.execute(
            select(MessageForSending).where(MessageForSending.user_id == create_new_user.user_id)
        )
    ...
    async with get_redis() as session_redis:
        redis_value = await session_redis.get(f"admin:{create_new_user.user_id}")
...
async def test_get_message_for_sending_returns_existing(create_admin_fix):
    ...
    message = await get_message_for_sending(create_admin_fix.user_id)
    assert isinstance(message, MessageForSending)

    # второй вызов должен вернуть ту же запись
    message_2 = await get_message_for_sending(create_admin_fix.user_id)
...
async def test_get_message_for_sending_creates_if_missing(create_admin_fix):
    ...
    async with get_db() as session_db:
        await session_db.execute(
            MessageForSending.__table__.delete().where(MessageForSending.user_id == create_admin_fix.user_id)
        )
        await session_db.commit()

    message = await get_message_for_sending(create_admin_fix.user_id)
    assert message is not None
    assert message.user_id == create_admin_fix.user_id

@pytest.mark.asyncio
async def test_update_message_for_sending_updates_data(create_admin_fix):
    updated = await update_message_for_sending(
        user_id=create_admin_fix.user_id,
        content="Hello World",
