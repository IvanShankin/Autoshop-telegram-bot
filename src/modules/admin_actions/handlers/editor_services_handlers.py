    ...
    if not service:
        if callback:
            try:
                ...
                pass
            await callback.answer(i18n.gettext("The service is no longer available"), show_alert=True)
            return

        await send_message(chat_id=user.user_id, message=i18n.gettext("The service is no longer available"))
        return
...
async def add_account_service(callback: CallbackQuery, user: Users):
    ...
    await edit_message(
        chat_id=callback.from_user.id,
        message_id=callback.message.message_id,
        message=i18n.gettext("Select service type"),
...
async def select_type_service(callback: CallbackQuery, state: FSMContext, user: Users):
    ...
    await edit_message(
        chat_id=callback.from_user.id,
        message_id=callback.message.message_id,
        message=i18n.gettext('Enter a name for the new service')
    )
...
async def get_service_name(message: Message, state: FSMContext, user: Users):
    ...
    try:
        ...
        await send_message(user.user_id, i18n.gettext("Unable to create. This service type is in use"))
        return

    await send_message(user.user_id, i18n.gettext("Service successfully created"), reply_markup=to_services_kb(user.language))
...
async def service_update_index(callback: CallbackQuery, user: Users):
    ...
    await callback.answer(i18n.gettext("Successfully updated"))
    await show_service(user=user, callback=callback, message_id=callback.message.message_id, service_id=service_id)
...
async def service_update_show(callback: CallbackQuery, state: FSMContext, user: Users):
    ...
    await edit_message(
        chat_id=callback.from_user.id,
        message_id=callback.message.message_id,
        message=i18n.gettext("Please enter a new name"),
...
async def update_service_name(message: Message, state: FSMContext, user: Users):
    ...
    try:
        ...
        info_message = await send_message(user.user_id, i18n.gettext("Name changed successfully"))
...
async def delete_acc_service(callback: CallbackQuery, user: Users):
    ...
    try:
        ...
        message = i18n.gettext("The service has categories, delete them first")
        reply_markup = back_in_service_kb(user.language, service_id)
