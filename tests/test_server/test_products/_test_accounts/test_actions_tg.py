...
class TestCheckValidAccounts:
    ...
    async def test_check_valid_accounts_success(self, monkeypatch, tmp_path):
        from src.application.products.accounts.tg._actions import check_valid_accounts_telethon
        ...
        from src.application.products.accounts.tg import _actions
...
async def test_check_account_validity_true(tmp_path, monkeypatch):
    ...
    from src.application.products.accounts.tg import _actions
    monkeypatch.setattr(actions, "decryption_tg_account", lambda account, crypto, status: folder_path)
    monkeypatch.setattr(actions, "TDesktop", lambda path: FakeTDesktop())

    acc = AccountStorage(account_storage_id=5)
    result = await check_account_validity(acc, AccountServiceType.TELEGRAM, acc.status)
...
async def test_get_auth_codes_reads_codes(tmp_path, monkeypatch):
    ...
    """
    from src.application.products.accounts.tg._actions import get_auth_codes
