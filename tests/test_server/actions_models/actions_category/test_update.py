...
class TestUpdateCategory:
    ...
    async def test_update_category_index_reorder_and_redis(self, create_category):
        ...
        async with get_db() as session:
            ...
            assert rows[0].category_id == c3.category_id
    ...
    async def test_update_category_changed_file_data(
        ...
        async with get_db() as session:
            res = await session.execute(
                select(Categories)
                .where(Categories.category_id == cat.category_id)
            )
            dbcat: Categories = res.scalar_one()
    ...
    async def test_update_account_category_validation_and_product_conflict(
        ...
        prod = await create_product_account(filling_redis=False, category_id=cat.category_id)
...
class TestUpdateAccountCategoryTranslation:
    ...
    async def test_update_account_category_translation_success(self,create_category):
        from src.application._database.categories.actions import update_category_translation
        ...
        async with get_db() as session:
            res = await session.execute(
                select(CategoryTranslation).where(
                    (CategoryTranslation.category_id == full_category.category_id) &
                    (CategoryTranslation.lang == "ru")
                )
            )
        ...
        async with get_redis() as r:
            ...
            found = [x for x in parsed_list if x["category_id"] == full_category.category_id]
